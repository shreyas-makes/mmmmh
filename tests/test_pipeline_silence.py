...
class SilenceCuttingTests(unittest.TestCase):
    ...
    def test_pause_floor_defaults_when_missing(self):
        cuts = build_cut_segments(
        ...
        self.assertEqual(len(cuts), 2)
        # 400ms silence with default 180ms floor means 220ms removed in the middle.
        self.assertAlmostEqual(cuts[0].end - cuts[0].start, 0.11, places=3)
        self.assertAlmostEqual(cuts[1].end - cuts[1].start, 0.11, places=3)
